# Cloud fire probe — plan & protocol

A throwaway, single-stage plan whose only purpose is to verify
`scripts/cloud_fire.py` end to end (issue #140): that a stage fired as a cloud
session actually starts on the plan branch, runs at the `model` and `effort`
its stage index row asks for, and can only push to the branches it was
permitted. The stage's deliverable **is** that evidence. Delete this branch once
the evidence is recorded. This file is the **single source of truth** for durable decisions: the
...
```
probe/
  cloud-fire-evidence.md   # what the fired session observed about itself
```
...
- **This plan is disposable.** It exists to prove the cloud fire path works and
  is deleted afterwards. Nothing here is a commitment about the plugin.
- **The stage touches nothing but `probe/`.** No plugin file, no script, no
  workflow, no README. A probe that changes the thing under test proves less,
  not more.
...
|---|---|---|---|---|---|---|---|
| S0 Fire evidence | `stage-0-fire-evidence.md` | — | direct | inline | haiku | med | auto |
