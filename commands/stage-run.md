...
# /stage-run — execute one stage

Run a single stage of an existing `.plan/`, in this session. This command is a **thin wrapper**:
...
6. **End announcement.** When you stop, state explicitly:
   ...
   - The **complete runnable set**: *every* `todo` stage whose `depends` are
     ...
     **`/plan-staged-rollout:stage-run <N>`** — and state its recommended
